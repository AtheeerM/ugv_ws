...
fixed_order_nav.py
==================
Visits the four waypoints in the fixed order they are defined in GOAL_DEFS —
no greedy scoring, no path-cost replanning, no goal reordering.

Companion to greedy_4_goals.py for side-by-side comparison.
Log files use the *_nogreedy suffix so both scripts can run in the same
~/robot_logs directory without overwriting each other.
"""
...
# _nogreedy suffix distinguishes these logs from greedy_4_goals logs
# ------------------------------------------------------------------
LOG_DIR          = os.path.expanduser("~/robot_logs")
NAV_LOG_FILE     = os.path.join(LOG_DIR, "navtraj_nogreedy.csv")
LORA_LOG_FILE    = os.path.join(LOG_DIR, "loratraj_nogreedy.csv")
NAV_LOG_INTERVAL = 5.0   # seconds between pose samples

# ------------------------------------------------------------------
# Goal definitions  — visited strictly in this order (index 0 → 3)
# ------------------------------------------------------------------
GOAL_DEFS = [
    {"x": -2.34,   "y":  1.32, "yaw": 0.0, "tag": "TAG_001"},
    {"x": 0.944,  "y": -2.38,  "yaw": 0.0, "tag": "TAG_002"},
    {"x": 1.71, "y": 0.978, "yaw": 0.0, "tag": "TAG_003"},
    {"x": -0.624, "y": -1.08, "yaw": 0.0, "tag": "TAG_004"},
]
...
class FixedOrder4Goals(Node):
    """Navigate to four goals in a fixed, predefined order."""

    def __init__(self):
        super().__init__("fixed_order_4_goals")
        self.map_frame  = "map"
        ...
        # Only NavigateToPose — no ComputePathToPose needed without greedy scoring
        ...
        # ── Goals — built once, visited front-to-back ───────────────
        self.goals = [
        ...
        time.sleep(15.0)
        self.get_logger().info("Starting fixed-order navigation!")
    ...
    def _init_log_files(self):
        """Create log directory and overwrite both files on startup."""
    ...
    def _mid_drive_check(self, is_home=False):
        """
        Only monitors recovery count — no path-cost comparison,
        no goal switching based on distance or cost.
        """
        ...
        if self._recovery_count >= MAX_RECOVERIES:
            self.get_logger().warn(
                f"Too many recoveries ({self._recovery_count}) — will retry after costmap clear")
    ...
    def _drive_with_monitoring(self, goal, is_home=False):
        ...
        while not result_event.wait(timeout=CHECK_INTERVAL):
            decision = self._mid_drive_check(is_home=is_home)
    ...
    def navigate_with_retry(
        ...
        """
        Keeps retrying the *same* goal until it succeeds or max_attempts
        is reached.  There is no 'switch_goal' / 'cheaper_found' path.
        """
        attempt        = 0
        ...
        while True:
            ...
            if result == 'succeeded':
                ...
                # Always retry the same goal — clear costmaps and try again
                if is_home:
                    self.get_logger().warn(
    ...
    # ------------------------------------------------------------------
    # Main navigation loop  — strict fixed order
    # ------------------------------------------------------------------
    ...
    def navigation_loop(self):
        ...
        goal_num = 0
        # Goals are visited front-to-back as listed in GOAL_DEFS
        while rclpy.ok() and self.goals:
            ...
            # Pop the next goal in fixed order (index 0)
            goal, expected_tag = self.goals.pop(0)
            ...
            self.clear_costmaps()
            ok, lora_confirmed = self.navigate_with_retry(
            ...
            if ok:
                ...
                # navigate_with_retry only returns False if max_attempts hit;
                # with max_attempts=None it never returns False, but guard here
                # anyway — requeue at the FRONT so order is preserved.
                self.get_logger().warn(
                    f"Could not reach {expected_tag} — "
                    f"requeueing at front to preserve fixed order...")
                self.goals.insert(0, (goal, expected_tag))
                self.clear_costmaps()
